...
{
    $result = $conn->query("SELECT code, tour_code, tourcode, wscode_tour, date_create, change_tm, api_updated_at FROM tour_online WHERE wholesale_code = 14  AND user_create = 'API GO 365'");
...
function insertSyncLogGo365($conn, $tour_id, $tour_code, $tour_date): void
{
...
function updateSyncLogGo365($conn, $tour_id, $tour_date_update, $action, $tour_code): void
{
...
function updateSyncLogGo365Delete($conn, $tour_id, $action, $tour_code): void
{
    $log = checkTourCodeLog($conn, $tour_id);
    if ($log) {
        $date_update = DateNow();
...
function checkTourCode($conn, $tour_code)
{
    $sql = "SELECT wscode_tour, change_tm, api_updated_at
            ...
            AND user_create = 'API GO 365'
            AND wholesale_code = 14";
...
{
    $stmt = $conn->prepare("
        SELECT * FROM sync_log_go365 WHERE tour_id = ? LIMIT 1
    ");
...
function getSyncLogGo365(mysqli $conn): array
{
    $sql = "SELECT * FROM sync_log_go365 WHERE is_synced = 0 ORDER BY created_at ASC LIMIT 5";
    $result = $conn->query($sql);

    if ($result === false) {
        error_log("Query failed (getSyncLogGo365): " . $conn->error);
        return [];
    }
...
function updateSyncLogGo365Finish(mysqli $conn, $code, $tour_code): void
{
    $dateUpdate = DateNow();
    $stmt = $conn->prepare(
        "UPDATE sync_log_go365 SET is_synced = 1, updated_at = ?, note = '' WHERE tour_id = ? AND tour_code = ?"
    );
    $stmt->bind_param("sss", $dateUpdate, $code, $tour_code);
    $stmt->execute();
    $stmt->close();
}

function updataSyncLogGo365Error(mysqli $conn, $code, $tour_code, $error): void
{
    $dateUpdate = DateNow();
    $stmt = $conn->prepare(
        "UPDATE sync_log_go365 SET is_synced = 1, updated_at = ?, note = ? WHERE tour_id = ? AND tour_code = ?"
    );
    $stmt->bind_param("ssss", $dateUpdate, $error, $code, $tour_code);
    $stmt->execute();
    $stmt->close();
}

...
function insertTour($conn, $detail, $tour_code): bool
{
    ...
    if (!empty($dataTour)) {
        ...
        $data_insert['wsname']           = 'Go 365';
        $data_insert['wholesale_code']   = '14';
        ...
        $data_insert['user_create']      = "API GO 365";
        $data_insert['date_create']      = DateNow();
        $data_insert['user_update']      = 'API GO 365';
    ...
    if (!empty($dataTourDay) && is_array($dataTourDay)) {
        ...
        foreach ($dataTourDay as $detailRow) {
            ...
            $user_create = 'API GO 365';
            $date_create = DateNow();
            $user_update = 'API GO 365';
            $date_update = DateNow();
    ...
    ");
        ...
        foreach ($dataTourPeriod as $periodRow) {
            ...
            $child_n = 0.00;
            $inf = 0.00;
            $priceStart = $periodRow['period_price_start'] ?? null;
            $rateSgl    = $periodRow['period_rate_adult_sgl'] ?? null;

            $single = (is_numeric($priceStart) && is_numeric($rateSgl))
                ? max(0, $rateSgl - $priceStart)
                : 0;
